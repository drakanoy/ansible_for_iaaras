...
#
# $Id: startdifx 9120 2019-08-22 02:43:35Z WalterBrisken $
# $HeadURL: $
# $LastChangedRevision: 9120 $
# $Author: WalterBrisken $
# $LastChangedDate: 2019-08-22 05:43:35 +0300 (Чт, 22 авг 2019) $
#
# ============================================================================
...
def watchJob(jobId):
    ...
    if port == None:
        print
        'DIFX_MESSAGE_PORT needs to be defined'
        exit(0)
    else:
        port = int(port)
    ...
    if group == None:
        print
        'DIFX_MESSAGE_GROUP needs to be defined'
        exit(0)
...
def getjobdifxversion(fileBase):
    ...
    if len(files) < 1:
        print
        'Can\'t find file %s.calc.  Quitting.' % fileBase
        exit(0);
...
def testDifxVersion(fileBase, override):
    ...
    if version == None:
        print
        'Warning: env. var. DIFX_VERSION not defined!'
    if jobversion == None:
        print
        'Warning: %s.calc does not contain version info' % fileBase
    if version != None and jobversion != None and version != jobversion:
        print
        'Warning: $DIFX_VERSION and job version (in .calc file) mismatch'
        print
        '    <%s> != <%s>' % (version, jobversion)
        if override:
            print
            'Continuing anyway due to --override-version'
        else:
            print
            'Quitting since --override-version not specified'
            exit(0)

    if label != '' and joblabel != '' and label != joblabel:
        print
        'Warning: $DIFX_LABEL and job label (in .calc file) mismatch'
        print
        '    <%s> != <%s>' % (ver, jobver)
...
def doMachines(fileBase, machinesPolicy, cache):
    ...
    if cache[2] == files:
        if verbose > 0:
            print
            'Using cached machines/threads info'
...
def runMessage(fileBase, machinesPolicy, deletePolicy, makeModel, override, useLocalHead, machinesCache,
               restartSeconds):
    ...
    if restartSeconds > 0:
        if not isdir(fileBase + '.difx'):
            print
            'Warning: restartSeconds = %d and no existing output!' % restartSeconds
    elif isdir(fileBase + '.difx') or isfile(fileBase + '.difx'):
        if deletePolicy == 1:
            if verbose > 0:
                print
                'Removing %s.difx' % fileBase
            sendMessage(fileBase, 'Info', 'Deleting %s.difx' % fileBase)
            system('rm -rf %s.difx' % fileBase)
        else:
            print
            'Warning: output file %s.difx exists' % fileBase
            return
...
def run(fileBase, machinesPolicy, deletePolicy, makeModel, override, useStartMessage, useLocalHead, machinesCache,
        restartSeconds):
    savedEnvironment = updateEnvironment(fileBase + '.input.env')
    if useStartMessage:
        rv = runMessage(fileBase, machinesPolicy, deletePolicy, makeModel, override, useLocalHead, machinesCache,
                        restartSeconds)
    else:
        rv = runDirect(fileBase, machinesPolicy, deletePolicy, makeModel, override, useLocalHead, machinesCache,
                       restartSeconds)
...
optParser.set_defaults(machinesPolicy=2, useStartMessage=False, deletePolicy=0, makeModel=True, difxdb=False,
                       makeFits=False, verbose=0, quiet=0, useLocalHead=False, override=False, machinesFile="")

optParser.add_option("-A", "--agent", dest="agent", action="store", type="string",
                     help="call mpirun through this agent with filebase as only argument")
optParser.add_option("-g", "--genmachines", dest="machinesPolicy", action="store_const", const=2,
                     help="will run genmachines even if not needed [default]")
optParser.add_option("-a", "--automachines", dest="machinesPolicy", action="store_const", const=1,
                     help="will run genmachines if needed")
optParser.add_option("-n", "--nomachines", dest="machinesPolicy", action="store_const", const=0,
                     help="will not run genmachines, even if needed")
optParser.add_option("-M", "--machines-file", dest="machinesFile", action="store", type="string",
                     help="use supplied machines file instead of one based on job name")
optParser.add_option("-m", "--message", dest="useStartMessage", action="store_true",
                     help="start difx via DifxStartMessage")
optParser.add_option("-f", "--force", dest="deletePolicy", action="store_const", const=1,
                     help="force running even if output file exists")
optParser.add_option("-d", "--dont-calc", dest="makeModel", action="store_false",
                     help="will not calculate delay model, even if needed")
optParser.add_option("-D", "--difxdb", dest="difxdb", action="store_true",
                     help="make use of difxdb to obtain module location")
optParser.add_option("-F", "--fits", dest="makeFits", action="store_true",
                     help="generate 1 fits file per job at end of each job")
optParser.add_option("-v", "--verbose", dest="verbose", action="count",
                     help="send more output to the screen and difxlog file (use -v -v for extra info)")
optParser.add_option("-q", "--quiet", dest="quiet", action="count", help="be quieter")
optParser.add_option("-l", "--localhead", dest="useLocalHead", action="store_true",
                     help="use the current host as the head node. Overrides DIFX_HEAD_NODE.")
...
if options.machinesFile != "" and isfile(options.machinesFile) == False:
    print
    "ERROR: The machines file (%s) does not exist" % (options.machinesFile)
    exit(1)

headNode = getenv('DIFX_HEAD_NODE')
if options.useStartMessage and options.useLocalHead == False and headNode == None:
    print
    "ERROR: Environment variable DIFX_HEAD_NODE must be defined."
    print
    "Alternatively  you can use the -l option in case mk5daemon -H is running locally on this host."
    exit(1)
...
for fileBase in fileBaseList:
    if not isfile(fileBase + '.input'):
        print
        'Error: %s.input not found (or not a regular file)' % fileBase
        nBad += 1
...
if dcp != None:
    ...
    print
    'Using specified calc program: %s' % delayModelProgram
    print
    ''
...
for fileBase in fileBaseList:
    ...
    if len(jobname) > 31:
        ...
        print
        "Job name %s is too long and will be skipped!" % fileBase
        exit(1)

    v = run(fileBase, options.machinesPolicy, options.deletePolicy, options.makeModel, options.override,
            options.useStartMessage,
