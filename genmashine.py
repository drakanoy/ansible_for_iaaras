...
#
# $Id: genmachines 9047 2019-07-05 23:30:07Z MarkWainright $
# $HeadURL: $
# $LastChangedRevision: 9047 $
# $Author: MarkWainright $
# $LastChangedDate: 2019-07-06 02:30:07 +0300 (Сб, 06 июл 2019) $
#
# ============================================================================
...
try:
    ...
    print
    "ERROR: Cannot find difxmachines library. Please include $DIFXROOT/lib/python in your $PYTHONPATH environment"
    sys.exit(1)
...
def convertDateString2Mjd(dateStr):
    ...
    if len(dateStr) == 18 and dateStr[4] == 'y' and dateStr[8] == 'd' and dateStr[11] == 'h' and dateStr[14] == 'm' and \
            dateStr[17] == 's':
...
def getVsnsByMulticast(maxtime, datastreams, verbose):
    ...
    while t < maxtime and len(missingVsns) > 0:
        try:
            ...
            print
            'Weird: cannot gethostbyaddr for %s' % address[0]
...
def startTimeInPackTimeRange(startTime, packStartStr, packEndStr):
    ...
    if startTime.days > packStartMjd.days or (
            startTime.days == packStartMjd.days and startTime.seconds >= packStartMjd.seconds):
        if startTime.days < packEndMjd.days or (
                startTime.days == packEndMjd.days and startTime.seconds <= packEndMjd.seconds):
            return True
    return False
...
def getvexantennamodlists(vexobsfile, startTime):
    ...
    if not experNameFound:
        print
        "could not find experiment name, exiting"
    if not tapelogObsFound:
        print
        "could not find TAPELOG_OBS data, exiting"
    if not ok:
        exit(1)
    # harmless lack of info
    if len(antennaModuleList) is 0:
        print
        "no antennas with mark6 modules found, nothing more to do, exiting"
        exit(0)
...
def adjustMark6datastreams(datastreams, inputfile, verbose, startTime):
    ...
    try:
        ...
        print
        calcfilename, "could not be openened, returning"
        return
    ...
    try:
        ...
        print
        vexobsfile, "could not be openened, returning"
        return

    if len(antennaModuleList) < 1:
        print
        "no antennas using mark6 found, returning"
        return
    ...
    for stream in datastreams:
        if stream.type == 'MARK6':
            ...
            while i < msnlen:
                ...
                if verbose > 0:
                    print
                    'trying to replace', stream.msn[i], 'with a valid mark6 msn'
                    ...
                    for antmod in antennaModuleList:
                        if ant == antmod[0]:
                            ...
                            if antmod[1] not in stream.msn:
                                if verbose > 0:
                                    print
                                    'replacing', stream.msn[i], 'with', antmod[1]
    ...
    for stream in datastreams:
        if stream.type == "FILE":
            ...
            if matchNode in difxmachines.getMk5NodeNames():
                ...
                print
                'stream type MODULE with VSN %s, matching node %s not listed as an active mark5 host in machines file' % (
                stream.vsn, matchNode)
                return []
            ...
            if matchNode in difxmachines.getMk6NodeNames():
                ...
                print
                'stream type MARK6 with MSN %s, matching node %s not listed as an active mark6 host in machines file' % (
                str(stream.msn), matchNode)
                return []
...
def run(infile, machinesfile, overheadcores, verbose, dothreads, useDifxDb):
    ...
    if not hostname in difxmachines.getHeadNodeNames():
        print
        'ERROR: hostname (%s) is not an allowed headnode in the machines file : %s' % (hostname, machinesfile)
        exit(1)
    ...
    if verbose > 0:
        ...
        for stream in datastreams:
            print
            'type', stream.type, 'vsn', stream.vsn, 'msn', stream.msn, 'path', stream.path

    if not uniqueVsns(datastreams):
        print
        'ERROR: at least one duplicate VSN exists in %s !' % infile
        exit(1)
    ...
    if verbose > 0:
        ...
        for r in results:
            if 'mark6' in r[0]:
                print
                '  %-10s : %10s %10s %10s %10s' % (
                r[0], r[2]['slot1MSN'], r[2]['slot2MSN'], r[2]['slot3MSN'], r[2]['slot4MSN'])
            else:
                print
                '  %-10s : %10s %10s   %s' % (r[0], r[2], r[3], r[4])
...
if __name__ == "__main__":
    ...
    parser.add_option("-m", "--machines", dest="machinesfile", default="",
                      help="use MACHINESFILE instead of $DIFX_MACHINES")
    parser.add_option("-n", "--nothreads", dest="dothreads", action="store_false", default=True,
                      help="don't write a .threads file")
    parser.add_option("-d", "--difxdb", dest="usedifxdb", action="store_true", default=False,
                      help="use difxdb to obtain data location")
    parser.add_option("--ignore-incomplete-module", dest="ignoreIncompleteModules", action="store_true", default=True,
                      help="Proceed even when Mark6 modules are found to be incomplete.")
    ...
    if quit:
        print
        'genmachines quitting.'
        exit(1)

    if verbose > 0:
        print
        'DIFX_MACHINES -> %s' % machinesfile
    ...
    print(machinesfile, difxmachines, 'ahahahahhahahhahahahahahhah')
    ...
    if fail:
        print
        "ERROR: This version of genmachines requires a cluster defintion file of version > %s. Found version is: %s.%s" % (
        minMachinefileVersion, major, minor)
        exit(1)
